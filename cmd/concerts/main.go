...
func main() {
	...
	// Serve swagger doc
	mux.Handle("GET", "/openapi3.yaml", http.HandlerFunc(http.FileServer(http.Dir("gen/http")).ServeHTTP))
	...
	port := "8080"
	server := &http.Server{Addr: ":" + port, Handler: corsHandler}
